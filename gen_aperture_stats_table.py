...
def gen_phys_props_table(
    ...
    t_phys['r_gal'] = r_gal = (
        mt['rang_gal'].quantity.to('rad').value *
        gal_dist).to('kpc')
    ...
    if np.isfinite(t_phys['Sigma_atom']).sum() == 0:
        t_phys['Sigma_atom'] = (
            mt['I_21cm_native'].quantity * gal_cosi * alpha21cm
        ).to('Msun pc-2')
    ...
    ICO10kpc = mt['I_CO21'].quantity * gal_cosi / R21
...
if __name__ == '__main__':
    ...
    for row in catalog:
        ...
        with fits.open(infile) as hdul:
            mt = TessellMegaTable(
                hdul[0].header, 
                cell_shape=aperture_shape, cell_size_deg=apersz_deg,
        ...
        radii, projang = deproject(
            center_ra=ra.value, center_dec=dec.value,
            incl=incl.value, pa=posang.value,
            ra=mt['RA'], dec=mt['DEC'])
        mt['rang_gal'] = (radii * u.deg).to('arcsec')
        mt['phi_gal'] = projang * u.deg
        # sort rows by galactic radii
        mt[:] = mt[np.argsort(mt['rang_gal'])]
        ...
        if np.isfinite(mt['I_CO21']).sum() == 0:
